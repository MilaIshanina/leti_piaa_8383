...
int getSuff(int top, std::vector<Top>& bor)//function for getting and setting suffix links
{
    ...
    {
        std::cout << "From vertex " << bor.at(top).numberSymbol << " a suffix link has not been made yet!" << std::endl;
        ...
        else
        {
            ...
            {
                std::cout << "Since the vertex " << bor.at(top).numberSymbol << " is a terminal vertex, a suffix reference to the beginning of the boron will be set for it!" << std::endl;
                bor.at(top).suffLink = 0;
            }
            else
            {
                std::cout << "For the current vertex " << bor.at(top).numberSymbol << " setting the suffix reference NOT to the beginning of the boron, but to ..." << std::endl;
                bor.at(top).suffLink = getAutoMove(getSuff(bor.at(top).parent, bor), bor.at(top).numberSymbol, bor);
                std::cout << "... on top " << bor.at(bor.at(top).suffLink).numberSymbol << std::endl;
            }
